...
fn bench_port_match(c: &mut Criterion) {
    let ranges: Vec<PortRange> = (80..90)
        .map(|p| PortRange::new(Port::new(p), Port::new(p)))
        .collect();
    let list = MemoryPortList::new(ranges);
    let hit_port = Port::new(85);

    c.bench_function("port_list_contains_10_ports_hit", |b| {
        b.iter(|| list.contains(hit_port))
    });
}
...
fn bench_port_miss(c: &mut Criterion) {
    ...
    c.bench_function("port_list_contains_10_ports_miss", |b| {
        b.iter(|| list.contains(miss_port))
    });
}
...
fn bench_port_range_match(c: &mut Criterion) {
    ...
    c.bench_function("port_range_contains_hit", |b| {
        b.iter(|| list.contains(hit_port))
    });
}
