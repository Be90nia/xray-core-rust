...
- **遗留(非阻塞, v53 口径)**: server 侧 inbound VisionConn splice **已合入**(v50/f926b06);ENC 0-RTT 双侧已实装(剩余=xor_mode==2 模式 Rust↔Go 互通对齐, 无真实节点);ss2022 多 PSK/重放/uot 多用户已测(uot 的 Go 真实 server 互操作不可行——Go 基准无此概念);grpc server 深层对称性仅三发验证;certificates 集成测试已补;trojan users 日志已同步;validator 拒绝日志已 AtInfo;**Rust↔Rust vision splice 512KB bulk 挂死已闭环**(v52 集成测试驱动形态对齐真实 pump 节奏, 非代码层问题);**本地双向互操作矩阵 12/12 全绿**(v53);workspace 画像全绿(v51 test3 复跑 0 failed, xray-common flaky 未复现);留账: xray-tls --features ocsp-stapling 既有编译错(ServerConfigAndStapler 缺 Debug)非新引入
