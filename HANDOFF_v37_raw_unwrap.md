...
- **遗留(非阻塞, v51 口径)**: server 侧 inbound VisionConn splice **已合入**(v50/f926b06);ENC 0-RTT 双侧已实装(剩余=xor_mode==2 模式 Rust↔Go 互通对齐, 无真实节点);ss2022 多 PSK/重放/uot 多用户已测(uot 的 Go 真实 server 互操作不可行——Go 基准无此概念);grpc server 深层对称性仅三发验证;certificates 集成测试已补;trojan users 日志已同步;validator 拒绝日志已 AtInfo;workspace 画像全绿(v51 test3 复跑 0 failed, xray-common flaky 未复现);**唯一开放缺陷**: Rust↔Rust start_full 拓扑 vision splice 512KB bulk 在 v51 TLS dirty 状态机修复后仍挂死(300s timeout 实测)——非 TLS 包装层单独问题, 疑 duplex+VisionConn server 模式+TLS 三层组合的第二处交互, **真实 TCP 部署不触发**(VPS 32/32 与 Go client 互操作全绿), 需专项会话深挖
