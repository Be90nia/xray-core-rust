...
- **✅ v43 测试债+矩阵广度收尾(2026-09-05, e7786d7+b470dbf)**: ① server 侧矩阵广度补全(Go client↔Rust server 四发: vless+vision 877KB/vmess 879KB/trojan 875KB 全 PASS 零修复;tuic 双侧一致移除——Rust server inbound 与 client outbound 矩阵对称覆盖); ② 两个挂起测试去 ignore 稳定绿(zero_rtt=测试脚本笔误非产品;relay_tunnel=顺带修 ss2022 inbound 读侧 nonce 断链真产品 bug——server 读 body 必 AEAD 失败,Rust↔Rust 自洽掩盖), #10 #26-28 补验全绿
- **workspace 全量测试画像(debug profile, D:/tmp/workspace_test2.txt)**: 30+ crate 全绿;已知失败 4 个全部定性既有测试债非回归——observatory×2+core observatory_init(block_on 无 runtime,8bef802 批未验证)+core integration_socks_udp_through_router(UDP 路由超时,5e03243 批引入时自注"cargo test 未验证");xray-proxy-ss 之后挂死=btls-sys debug profile 首编 NASM 坑(DnsFamilyFix 沉淀有配方,非测试问题)
- **遗留(非阻塞, v43 口径)**: server 侧 inbound VisionConn splice/ENC 0-RTT server 会话未实现(outbound 全通,0-RTT client 快路径已启用);zero_rtt wire 单测已修挂起为稳定绿(遗留=product 深层 duplex 交互理论仍可挖);服务级 query_strategy s.ipOption 钳制 Rust 无对应(Go dns.go:228-229);ss2022 多 PSK/重放防护未测;grpc server 深层对称性仅三发验证;**workspace 4 个既有测试失败待修**(observatory block_on×2+core observatory_init+core integration_socks_udp,均非协议链路);Rust certificates[] 非法内联静默回退自签无告警(ServerMatrix 发现的行为偏差)
