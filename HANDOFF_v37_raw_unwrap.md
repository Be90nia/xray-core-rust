...
- **✅ v45 分诊清偿轮(2026-09-05, 9587026+d78649f)**: 10 个 debug-profile 既有失败全清——8 测试烂(params 数据 1579 字符/vision mock 旧线型/btls_reality 钉死已删 BIO 行为/hysteria 绕过 varint 前缀)+**2 产品真协议偏差**(xor_mode>0 relay XOR 整段缺失 Go client.go:98-99+XorConn 读写 CTR 对调 Go server.go:325, 生产全 native(0) 未触达已修); certificates 对齐 Go error 语义(非法内联不再静默回退自签); dns 服务级 query_strategy 双分支钳制(UseSys→check_routes/其余 per-query AND, Go dns.go:223-230 两层防御与 Client 层不等价); 各 crate --lib 全绿, PM 复跑全套 32/32 零回归(REALITY 四节点满额); workspace 完整画像因 xray-common 疑似 flaky 挂死未出(与改动零交集, 留 Main 复跑)
...
- **遗留(非阻塞, v45 口径)**: server 侧 inbound VisionConn splice/ENC 0-RTT server 会话未实现(outbound 全通,0-RTT client 快路径已启用);ss2022 多 PSK/重放防护未测;grpc server 深层对称性仅三发验证;Rust certificates[] 非法内联已改 Err 但集成测试未跑;workspace 完整画像待复跑(xray-common 疑似 flaky 挂死);xor_mode>0 修复后无真节点验证(生产全 native(0),语义对齐 Go 已单测覆盖)
