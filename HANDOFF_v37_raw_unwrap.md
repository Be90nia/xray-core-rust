...
- **遗留(非阻塞, v54 口径)**: ss2022 uot = Go 无此概念的 Rust 扩展, **按用户裁决口径 Rust↔Rust 闭环达成**(2026-09-05): 双实例真实链 Rust client(ss2022 outbound uot:true,uotVersion:1)→Rust server(ss2022 inbound)→curl YouTube = **HTTP 200 + 875449B + YouTube marker**(与 32 套同验收口径, D:/tmp/uot_*.json 配置形态), UDP dispatch 回环另有单测覆盖(ss2022_uot_multi_e2e 1/0); grpc server 已双向多请求闭环(v54); ocsp-stapling 已修(v54); xor_mode==2 已双向互通(v54); REALITY/vision server 穿透已本机实证(v54, Docker Linux 床配方见容器章节); workspace 画像全绿; **注脚清零——本机可验项全部闭环**; 留账: xicmp linux_impl 仍为 ponytail 占位(send/recv 返回 Unsupported, 与 Windows 路径行为一致); Rust client grpc 无连接池(单连接多请求仅 Go client 方向结构性成立); EncXor2 residual: xor_mode==2 的 0-RTT 双向未单独立项测(client 0-RTT deferred/server (16,0) 已按 Go 对齐, Rust↔Rust zero_rtt 绿); Docker 调试床配方: `docker run -d --name xrbed2 -p 18400:18400 -v D:/Project/Xray-core-rust:/src -v D:/tmp:/tmp_host rust:1-bookworm sleep infinity` + `docker cp` 拷 `/lt/release/xray`(Linux 编译须 CARGO_TARGET_DIR=/lt 隔离+btls-sys 走 BORING_BSSL_SOURCE_PATH=/src/target/boringssl-patched+BORING_BSSL_ASSUME_PATCHED=1, 容器 config.toml [env] 固化)
