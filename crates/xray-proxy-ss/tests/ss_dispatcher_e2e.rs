...
use xray_proxy_ss::inbound::SsInbound;
use xray_proxy_ss::outbound::SsOutbound;
use xray_proxy_ss::stream::SSStream;
...
async fn outbound_inbound_handshake_metadata_matches() {
    ...
    let _out_stream: SSStream<TcpStream> = ob
        .process(&target_addr, target_port)
        .await
        .expect("outbound process");

    // 等待 inbound 解析完成
    let (header, _in_stream) = server_handle
        .await
        .expect("server task join")
        .expect("handshake should succeed");
    assert_eq!(header.port, target_port, "inbound 还原的 port 应与 outbound 写入一致");
    assert_eq!(
        header.address, target_addr,
        "inbound 还原的 address 应与 outbound 写入一致"
    );
}
...
/// 2. SS inbound 适配器：`handle_conn` 解密首帧得到目标 → 用裸 `TcpStream` 拨号到 echo
///    （等价 freedom outbound 对 IP 目标）→ `read_chunk` 读 body → 转发到 echo →
///    读回 echo → `write_chunk` 加密写回客户端
...
async fn outbound_inbound_full_chain_to_echo_roundtrip() {
    ...
    let server_handle = tokio::spawn({
        ...
        async move {
            ...
            let body = ss_stream
                .read_chunk()
                .await
                .expect("read body chunk")
                .expect("non-empty body");

            let mut echo_conn = TcpStream::connect(target_socket_addr)
                .await
                .expect("connect echo");
    ...
    let mut stream = ob
        .process(&Address::IPv4(echo_v4), echo_addr.port())
        .await
        .expect("outbound process");
    ...
    assert_eq!(
        &resp[..],
        payload,
        "echo through outbound→inbound→echo chain should match"
    );
