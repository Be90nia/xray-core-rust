...
pub fn parse_domain_rules(
    ...
    rules.iter().map(|rule| parse_domain_rule(rule, default_type, datadir)).collect()
}
...
/// 解析自定义 IP 规则（CIDR 格式）。
fn parse_custom_ip_rule(rule: &str, reverse: bool) -> Result<IpRule, RuleParserError> {
    let cidr = parse_cidr(rule)?;
    Ok(IpRule {
        value: Some(IpRuleValue::Custom(CidrRule { cidr: Some(cidr), reverse_match: reverse })),
    })
}
...
pub fn parse_cidr(s: &str) -> Result<Cidr, RuleParserError> {
    ...
    let ip: IpAddr =
        ip_str.parse().map_err(|_| RuleParserError::InvalidIPAddress(ip_str.to_string()))?;
    ...
    if prefix > max_prefix {
        return Err(RuleParserError::CidrPrefixTooLarge { prefix, max: max_prefix });
    }
...
pub fn parse_custom_domain_rule(
    ...
    if let Some(value) = rule.strip_prefix("regexp:") {
        return Ok(DomainRule { value: Some(DomainRuleValue::Custom(Domain::regex(value))) });
    }
    if let Some(value) = rule.strip_prefix("domain:") {
        return Ok(DomainRule { value: Some(DomainRuleValue::Custom(Domain::domain(value))) });
    }
    if let Some(value) = rule.strip_prefix("full:") {
        return Ok(DomainRule { value: Some(DomainRuleValue::Custom(Domain::full(value))) });
    }
    if let Some(value) = rule.strip_prefix("keyword:") {
        return Ok(DomainRule { value: Some(DomainRuleValue::Custom(Domain::substr(value))) });
    }
    if let Some(value) = rule.strip_prefix("dotless:") {
        return parse_dotless_rule(value);
    }

    Ok(DomainRule { value: Some(DomainRuleValue::Custom(Domain::new(default_type as i32, rule))) })
}
...
fn parse_dotless_rule(value: &str) -> Result<DomainRule, RuleParserError> {
    if value.is_empty() {
        return Ok(DomainRule { value: Some(DomainRuleValue::Custom(Domain::regex("^[^.]*$"))) });
    }
    if value.contains('.') {
        return Err(RuleParserError::DotlessContainsDot(value.to_string()));
    }
    let pattern = format!("^[^.]*{}[^.]*$", value);
    Ok(DomainRule { value: Some(DomainRuleValue::Custom(Domain::regex(&pattern))) })
}

/// 检查 dat 文件中是否存在指定代码的条目。
///
/// 如果 `check_file` 失败，转为 [`RuleParserError::FileCheckFailed`] 错误。
fn check_file(datadir: &Path, filename: &str, code: &str) -> Result<(), RuleParserError> {
    let loader = GeoDataLoader::new(datadir.to_path_buf());
    loader.check_file(filename, code).map_err(|e| RuleParserError::FileCheckFailed(e.to_string()))
}
...
mod tests {
    ...
    fn test_cut_reverse_prefix() {
        ...
        assert_eq!(cut_reverse_prefix("!ext:geoip.dat:cn"), ("ext:geoip.dat:cn", true));
    }
    ...
    fn test_parse_cidr() {
        ...
        assert!(matches!(result, Err(RuleParserError::CidrPrefixTooLarge { prefix: 33, max: 32 })));

        // 无效 IP
        let result = parse_cidr("invalid/8");
        assert!(matches!(result, Err(RuleParserError::InvalidIPAddress(_))));

        // 无效前缀
        let result = parse_cidr("10.0.0.0/abc");
        assert!(matches!(result, Err(RuleParserError::InvalidCidrPrefix(_))));
    }
    ...
    fn test_parse_custom_domain_rule() {
        // regexp:
        let rule =
            parse_custom_domain_rule("regexp:.*\\.example\\.com", DomainType::Domain).unwrap();
        match rule.value {
        ...
        // domain:
        let rule = parse_custom_domain_rule("domain:example.com", DomainType::Domain).unwrap();
        match rule.value {
            Some(DomainRuleValue::Custom(d)) => {
        ...
        // full:
        let rule = parse_custom_domain_rule("full:www.example.com", DomainType::Domain).unwrap();
        match rule.value {
            Some(DomainRuleValue::Custom(d)) => {
        ...
        // keyword:
        let rule = parse_custom_domain_rule("keyword:example", DomainType::Domain).unwrap();
        match rule.value {
            Some(DomainRuleValue::Custom(d)) => {
        ...
        // 无前缀，使用 default_type
        let rule = parse_custom_domain_rule("example.com", DomainType::Full).unwrap();
        match rule.value {
            Some(DomainRuleValue::Custom(d)) => {
                assert_eq!(d.r#type, DomainType::Full as i32);
                assert_eq!(d.value, "example.com");
    ...
    fn test_parse_custom_domain_rule_dotless() {
        // dotless: 空串 → "^[^.]*$"
        let rule = parse_custom_domain_rule("dotless:", DomainType::Domain).unwrap();
        match rule.value {
            Some(DomainRuleValue::Custom(d)) => {
                assert_eq!(d.r#type, DomainType::Regex as i32);
                assert_eq!(d.value, "^[^.]*$");
        ...
        // dotless: 无点子串
        let rule = parse_custom_domain_rule("dotless:example", DomainType::Domain).unwrap();
        match rule.value {
            Some(DomainRuleValue::Custom(d)) => {
                assert_eq!(d.r#type, DomainType::Regex as i32);
                assert_eq!(d.value, "^[^.]*example[^.]*$");
        ...
        // dotless: 有点子串 → 错误
        let result = parse_custom_domain_rule("dotless:example.com", DomainType::Domain);
        assert!(matches!(result, Err(RuleParserError::DotlessContainsDot(_))));
    }
    ...
    fn test_parse_ip_rules_reverse() {
        ...
        let result = parse_ip_rules(&rules, Path::new("/nonexistent")).unwrap();
    ...
    fn test_parse_domain_rules() {
        ...
        let result =
            parse_domain_rules(&rules, DomainType::Domain, Path::new("/nonexistent")).unwrap();
