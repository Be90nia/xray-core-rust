...
async fn http_connect_proxy_to_echo_target_e2e() {
    ...
    tokio::spawn(async move {
        ...
        let hs = http_server_handshake(&mut client_stream, &config)
            .await
            .expect("handshake");
        assert_eq!(hs.method, "CONNECT");
        let target_conn = dial_system(&hs.dest, &SocketOptions::default())
            .await
            .expect("dial target");
...
async fn http_plain_proxy_get_forwarded_e2e() {
    ...
    tokio::spawn(async move {
        ...
        // 拨号目标 + 写请求 + 读响应 → 转发客户端
        let mut target = dial_system(&hs.dest, &SocketOptions::default())
            .await
            .expect("dial target");
