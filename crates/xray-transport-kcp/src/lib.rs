...
//!
//! - **Segment 协议**（[`segment`]）：`DataSegment` / `AckSegment` / `CmdOnlySegment` 的二进制
//!   serialize/parse，与 xray Go 端字节级兼容。
//! - **State 状态机**（[`state`]）：6 态 `Active / ReadyToClose / PeerClosed / Terminating /
//!   PeerTerminated / Terminated` + `Is(...)` 谓词。
//! - **RoundTripInfo**（[`round_trip`]）：RFC 6298 RTT 估算（SRTT + RTTVAR + RTO）。
//! - **Config**（[`config`]）：包裹 prost 生成的 `Config`，提供 `GetSendingInFlightSize` 等派生计算
//!   + 默认配置。
//! - **SendingWorker / SendingWindow**（[`sending`]）：发送窗口、ACK 处理、丢包调整。
//! - **ReceivingWorker / ReceivingWindow / AckList**（[`receiving`]）：接收窗口、 ACK
//!   列表合并刷新。
...
//!   `internet.DialSystem`（实际 UDP 连接）+ `tls.Client`（TLS 包装）+ `UdpmaskManager`（UDP
//!   masking）。当前仅定义 trait + 编排框架（构造 Connection
//!   + spawn 输入循环 + 可选 TLS 套壳），实际 UDP/TLS/Udpmask 留待上层注入。
//! - **Listener**（[`listener`]）：`KcpListenerFactory` trait。Go `Listener` 依赖 `udp.Hub`（UDP
//!   包接收循环）+ `tls.Server` + `internet.ConnHandler`。当前仅 定义 trait + 编排框架（sessions
//!   map + OnReceive + Remove），实际 UDP socket bind + 接收循环留待上层注入。
//!
//! # 不引入 `kcp-tokio` 等通用 KCP crate
//!
//! xray mKCP 是 **skywind3000 KCP 协议的 xtaci Go 端口的 xray 定制版**：
//!
//! - 自定义 Segment 二进制格式（DataSegment: 18B overhead + payload； AckSegment: 17B +
//!   N×4B；CmdOnlySegment: 16B 固定）
...
pub use connection::{ConnMetadata, Connection, ConnectionCloser, KcpConn, NoopCloser};
...
pub use segment::{
    AckSegment, CmdOnlySegment, Command, DATA_SEGMENT_OVERHEAD, DataSegment, Segment, SegmentKind,
    SegmentOption, read_segment,
};
pub use state::{STATE_ACTIVE, State};
