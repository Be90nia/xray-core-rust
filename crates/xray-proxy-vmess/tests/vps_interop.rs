...
async fn vmess_ws_tls_vps_interop() {
    ...
    let tls = utls::client(conn, ws_host, tls_config)
        .await
    ...
    let (mut ws, response) = tokio_tungstenite::client_async(ws_request, tls)
        .await
        .expect("WS upgrade");
    eprintln!(
        "  WS upgrade response: {}",
    ...
    session
        .encode_request_body(&header, http_request, &mut body_bytes)
        .expect("encode body");
    ...
    eprintln!(
        "  Sending {} bytes (header={} + body={})",
        payload.len(), header_len, body_len
    );
