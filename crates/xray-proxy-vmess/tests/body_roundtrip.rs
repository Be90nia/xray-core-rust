...
fn full_roundtrip_request_response_cycle() {
    ...
    let sealed_header =
        client.encode_request_header(&request_header, &cmd_key).expect("encode_request_header");
    ...
    let (decoded_req_header, matched_user) =
        server.decode_request_header(&mut reader).expect("decode_request_header");
    ...
    let decoded_req_body =
        server.decode_request_body(&decoded_req_header, &mut reader).expect("decode_request_body");
    ...
    let decoded_resp_header =
        client.decode_response_header(&mut client_reader).expect("decode_response_header");
...
fn replay_same_request_header_fails() {
    ...
    assert!(
        matches!(
            err,
            xray_proxy_vmess::error::VmessError::Replay
                | xray_proxy_vmess::error::VmessError::DuplicateSession
        ),
...
fn decode_request_body_wrong_key_fails() {
    ...
    let err = server.decode_request_body(&decoded_req_header, &mut reader).unwrap_err();
    assert!(
...
fn full_roundtrip_chacha20poly1305() {
    ...
    let mut client_reader = &server_to_client[..];
    let _ = client.decode_response_header(&mut client_reader).expect("decode_response_header");
    let decoded_resp_body = client
        .decode_response_body(&request_header, &mut client_reader)
        .expect("decode_response_body");
    assert_eq!(decoded_resp_body, response_payload);
...
fn full_roundtrip_chunk_masking() {
    ...
    request_header.option.set(xray_proxy_vmess::request_option::CHUNK_MASKING);
    ...
    let (decoded_req_header, _user) =
        server.decode_request_header(&mut reader).expect("decode_request_header");
    assert!(decoded_req_header.option.has(xray_proxy_vmess::request_option::CHUNK_MASKING));
    let decoded_req_body =
        server.decode_request_body(&decoded_req_header, &mut reader).expect("decode_request_body");
    assert_eq!(decoded_req_body, request_payload);
