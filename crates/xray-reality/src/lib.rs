...
//! - [`crypto`]：协议算法（session_id 编码、ECDH auth_key 派生、AES-GCM 加密、 HMAC-SHA512
//!   证书验证），独立可测
//! - [`client`]：[`u_client`](client::u_client) 工厂——btls 浏览器指纹主路径
//!   （`xray_tls::btls_reality::connect_reality`）+ watfaq-rustls fallback（指纹不被 btls
//!   支持时走标准 rustls ClientHello）
//! - [`server`]：[`server_tls`](server::server_tls)（rustls 默认）与
//!   [`server_tls_btls`](server::server_tls_btls)（bd tce2 opt-in，BoringSSL 握手 + 26zn
//!   后握手记录模仿消费）——读 ClientHello record → REALITY 验证（共享前置）→
//!   伪造证书握手（[`RealityTlsStream`](server::RealityTlsStream) 双路）；验证失败返回
...
//!   （[`mitm::generate_reality_ed25519_cert_mldsa65`]，契约测试覆盖），客户端 btls ServerHello
//!   捕获 + 验签链路已落地（cm97）；但 rustls `ResolvesServerCert::resolve()` 拿不到 ServerHello
...
pub use config::{
    LimitFallback, MaxUselessRecordsSetting, RealityConfig, SHORT_ID_LEN, ServerAcceptorSetting,
    ShortId, X25519_KEY_LEN,
};
pub use crypto::{
    AEAD_NONCE_LEN, AUTH_KEY_LEN, HKDF_INFO, HKDF_SALT_LEN, MLDSA65_PUBKEY_LEN, MLDSA65_SEED_LEN,
    MLDSA65_SIG_LEN, SESSION_ID_LEN, derive_auth_key, derive_mldsa65_pubkey, encode_session_id,
    encrypt_session_id, verify_mldsa65_signature, verify_reality_certificate,
};
