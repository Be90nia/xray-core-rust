...
mKCP 是 UDP 传输, 跑在 server/client 进程的 UDP socket 上; socks5h TCP 经
vless/vmess+kcp outbound 整流到 UDP, 再经 inbound 解包. Go/Rust 均用 alterId=0.

证书非必需（明文 UDP）, 此处仍生成 RSA 证书便于将来扩展 mKCP+TLS 子用例.
"""
...
def kcp_stream():
    # Go KCPConfig 字段: mtu/tti/uplinkCapacity/downlinkCapacity/cwndMultiplier/maxSendingWindow/header/seed.
    # 默认 mtu=1350 (以太网典型值), tti=50ms (Go/Rust 均 10~1000 内), bandwidth=5MB/s.
    return {"network": "kcp", "kcpSettings": {"mtu": 1350, "tti": 50,
                                              "uplinkCapacity": 5, "downlinkCapacity": 5}}
...
def build(idx, server_bin, client_bin):
    ...
    P = 18500 + idx * 2
    S = P + 1
    # vmess+kcp: alterId=0 (AEAD), security="auto" 兼容 Go/Rust
    si = {"port": P, "listen": "127.0.0.1", "protocol": "vmess",
          "settings": {"clients": [{"id": UUID, "alterId": 0}]},
          "streamSettings": kcp_stream()}
...
def run_one(idx, server_bin, client_bin, tag):
    ...
    kill()
    print(f'[{flag}] #{idx:02d} {tag:14s} {"vmess/kcp":12s} HTTP body={bs:7d}B {err}', flush=True)
    return flag
...
if __name__ == '__main__':
    results = []
    idx = 1
    print('== A: Rust client -> Rust server (mock 对照, vmess+kcp 必通) ==', flush=True)
